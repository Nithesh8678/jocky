...
from sqlalchemy import create_engine, String, DateTime, JSON, ForeignKey, Text, Integer, Boolean, UniqueConstraint
...
class CaseDetection(Base):
    ...
    case_id: Mapped[str] = mapped_column(ForeignKey('cases.id'), primary_key=True)
    detection_id: Mapped[str] = mapped_column(ForeignKey('detections.id'), primary_key=True)
