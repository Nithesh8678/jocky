    ...
    while i < c.len() {
        ...
        if ch == '"' {
            ...
            while i < c.len() {
                let cc = c[i];
                if cc == '\n' {
                    return Err(format!("{line}:{sc}: newline in string"));
                }
                i += 1;
                if cc == '"' && !escaped {
                    break;
            ...
            if c.get(i - 1) != Some(&'"') || i == start + 1 {
                return Err(format!("{line}:{sc}: unterminated string"));
            }
            kind = "string";
            ...
            return Err(format!("{line}:{col}: unexpected character {ch}"));
        }
